"""Congo Republic — Min. Hydrocarbures Congo-Brazza  Currency: XAF"""
import re, sys
...
class CongoRepublicScraper(SmartScraper):
    COUNTRY_META = {
        "country":"Congo Republic","iso2":"CG","region":"Central Africa","currency":"XAF",
        "update_cycle":CYCLE_QUARTERLY,"price_range":(400,1200),
        "official_sources":[
            {"url":"https://www.hydrocarbures.gouv.cg/","name":"Min. Hydrocarbures Congo-Brazza","confidence":"high"},
            {"url":"https://www.hydrocarbures.gouv.cg/prix/","name":"Min. Hydrocarbures Congo-Brazza alt","confidence":"high"},
        ],
        "search_queries":[
            "Congo Republic prix carburant FCFA XAF litre {month} {year} officiel",
            "Congo Republic fuel price XAF FCFA {year} official",
        ],
        "last_known":{"gas_loc":695,"die_loc":630,"date":"2026-01-01"},
    }
...
def scrape(): return CongoRepublicScraper().run()
if __name__=="__main__":
    r=scrape(); print(f"Congo Republic: gas={r.gas_loc} XAF | die={r.die_loc} XAF | {r.effective_date}")

"""Benin — Min. Énergie Bénin  Currency: XOF"""
import re, sys
...
class BeninScraper(SmartScraper):
    COUNTRY_META = {
        "country":"Benin","iso2":"BJ","region":"West Africa","currency":"XOF",
        "update_cycle":CYCLE_MONTHLY_ANY,"price_range":(400,1200),
        "official_sources":[
            {"url":"https://www.gouv.bj/","name":"Min. Énergie Bénin","confidence":"high"},
            {"url":"https://www.gouv.bj/actualites/","name":"Min. Énergie Bénin (fallback)","confidence":"high"},
        ],
        "search_queries":[
            "Benin prix carburant FCFA XOF litre {month} {year} officiel arrêté",
            "Benin fuel price XOF FCFA {year} official decree",
        ],
        "last_known":{"gas_loc":740,"die_loc":760,"date":"2026-02-01"},
    }
...
def scrape(): return BeninScraper().run()
if __name__=="__main__":
    r=scrape(); print(f"Benin: gas={r.gas_loc} XOF | die={r.die_loc} XOF | {r.effective_date}")

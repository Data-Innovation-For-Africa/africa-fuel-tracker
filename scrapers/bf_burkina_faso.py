"""Burkina Faso — SONABHY  Currency: XOF"""
import re, sys
...
class BurkinaFasoScraper(SmartScraper):
    COUNTRY_META = {
        "country":"Burkina Faso","iso2":"BF","region":"West Africa","currency":"XOF",
        "update_cycle":CYCLE_MONTHLY_ANY,"price_range":(400,1200),
        "official_sources":[
            {"url":"https://www.sonabhy.bf/","name":"SONABHY","confidence":"high"},
            {"url":"https://www.sonabhy.bf/prix-carburants/","name":"SONABHY (fallback)","confidence":"high"},
        ],
        "search_queries":[
            "Burkina Faso prix carburant FCFA XOF litre {month} {year} officiel arrêté",
            "Burkina Faso fuel price XOF FCFA {year} official decree",
        ],
        "last_known":{"gas_loc":840,"die_loc":724,"date":"2026-02-01"},
    }
...
def scrape(): return BurkinaFasoScraper().run()
if __name__=="__main__":
    r=scrape(); print(f"Burkina Faso: gas={r.gas_loc} XOF | die={r.die_loc} XOF | {r.effective_date}")

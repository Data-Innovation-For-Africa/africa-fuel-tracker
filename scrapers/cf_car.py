"""CAR — Primature CAR  Currency: XAF"""
import re, sys
...
class CARScraper(SmartScraper):
    COUNTRY_META = {
        "country":"CAR","iso2":"CF","region":"Central Africa","currency":"XAF",
        "update_cycle":CYCLE_QUARTERLY,"price_range":(600,2000),
        "official_sources":[
            {"url":"https://www.primature.cf/","name":"Primature CAR","confidence":"high"},
            {"url":"https://mines-car.cf/","name":"Primature CAR alt","confidence":"high"},
        ],
        "search_queries":[
            "CAR prix carburant FCFA XAF litre {month} {year} officiel",
            "CAR fuel price XAF FCFA {year} official",
        ],
        "last_known":{"gas_loc":1050,"die_loc":1300,"date":"2026-01-05"},
    }
...
def scrape(): return CARScraper().run()
if __name__=="__main__":
    r=scrape(); print(f"CAR: gas={r.gas_loc} XAF | die={r.die_loc} XAF | {r.effective_date}")

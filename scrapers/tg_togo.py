"""Togo — Min. Mines Énergie Togo  Currency: XOF"""
import re, sys
...
class TogoScraper(SmartScraper):
    COUNTRY_META = {
        "country":"Togo","iso2":"TG","region":"West Africa","currency":"XOF",
        "update_cycle":CYCLE_MONTHLY_ANY,"price_range":(400,1100),
        "official_sources":[
            {"url":"https://www.mines-energie.gouv.tg/","name":"Min. Mines Énergie Togo","confidence":"high"},
            {"url":"https://www.mines-energie.gouv.tg/prix/","name":"Min. Mines Énergie Togo (fallback)","confidence":"high"},
        ],
        "search_queries":[
            "Togo prix carburant FCFA XOF litre {month} {year} officiel arrêté",
            "Togo fuel price XOF FCFA {year} official decree",
        ],
        "last_known":{"gas_loc":737,"die_loc":713,"date":"2026-02-01"},
    }
    ...
    def _xof(self, t, kws):
        for kw in kws:
            ...
            if m:
                v=float(m.group(1)); return v if 400<=v<=1100 else None
...
def scrape(): return TogoScraper().run()
if __name__=="__main__":
    r=scrape(); print(f"Togo: gas={r.gas_loc} XOF | die={r.die_loc} XOF | {r.effective_date}")
